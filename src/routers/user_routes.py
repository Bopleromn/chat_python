...
async def handle_user_get(email: str, password: str, db: Session = Depends(get_db)):
    user = db.query(UsersTable).filter(UsersTable.email == email).filter(
        UsersTable.password == password).first()
...
async def hande_user_put(email: str, password: str, new_user: UserReqModel, db: Session = Depends(get_db)):
    ...
    try:
        ...
        updated_user = db.query(UsersTable).filter(UsersTable.email == new_user.email).filter(
            UsersTable.password == new_user.password).first()
...
async def handle_verification_code_check(email: str, verification_code: str, db: Session = Depends(get_db)):
    result = db.query(VerificationCodesTable).filter(VerificationCodesTable.email == email).filter(
        VerificationCodesTable.verification_code == verification_code).first()
